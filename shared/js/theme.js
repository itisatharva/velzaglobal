...
$(document).ready(function () {
    ...
    $(MenuCSSselector).hcOffcanvasNav({
        // width: '100vw';
        disableAt: 9999 /* menu crossover — pushed above all real screen widths so the hamburger/off-canvas menu is used at every size */,
        width: 290,
        disableBody: false,
        insertClose: false,
        insertBack: true,
    ...
    function massFadeIn(
        ...
        $(theTrigger).each(function (i) {
            ...
            if (finalTrigger.length) {
                ...
                if (finalTarget.length) {
                    // if trigger.length done at the start
    ...
    function animatedHeadlines($targetSelector) {
        ...
        $($targetSelector).each(function () {
            ...
            if (words.length) {
                ...
                element
                    .find(".sk__animated-headline-words span.lvl2.sk__visible")
                    .wrap("<b></b>");
    ...
    function parallaxHeader() {
        $(".sk__parallax-header").each(function () {
            ...
            let parallaxHeaderImage = $(this).find(
    ...
    function equalizeHeroBoxesHeights() {
        ...
        $(".sk__hero-section .carousel-item").each(function () {
            ...
            if (w >= 1800) {
                ...
                $(this)
                    .find(".flex-helper-div")
                    .css("height", heroBottomLeftDivHeight * 0.82 + "px");
            } else {
                $(this)
                    .find(".flex-helper-div")
                    .css("height", heroBottomLeftDivHeight * 0.2 + "px");
            }
        });
    }
    ...
    function hugeDecorativeVerticalTexts() {
        ...
        if (ratio > 1) {
            $(".sk__huge-vertical-deco-text-section").each(function () {
                let trigger = $(this);
                let hugeTextVrapper = $(this).find(
                    ".sk__huge-vertical-deco-text-wrapper",
                );
                let hugeVerticalText = $(this).find(
    ...
    $(".sk__project-gallery").each(function (i) {
        ...
        $(items).each(function (i) {
            $(this).attr(
                "id",
                "sk__gallery-" + galleryNumber + "-item-" + (i + 1),
            );
        });
    ...
    $(
        ...
        if (classes.length) {
            ...
            $(
                '<span class="' + newClasses + '"> ' + insideHTML + "</span>",
            ).insertAfter(this);
        }
    });
    ...
    function iconBoxElementsEntrance($element) {
        let element = $element;
        let iconBoxTL = gsap.timeline().fromTo(
            element,
            {
                x: -100,
                opacity: 0,
    ...
    if (projectHeaderSection.length) {
        ...
        if (isMobile != true) {
            ...
            let phsThumbnailLandscapeCover = $(
            ...
            if (
                ...
                let phsThumbnailLandscape = gsap
                    .timeline()
    ...
    $(".sk__hero-section .carousel-item .sk__video-background-section").each(
        function () {
    ...
    if (isMobile != true) {
        ...
        // }
        // About section parallax (supports multiple About sections)
        var parallaxLayersAbout = document.querySelectorAll(
            ".sk__rectangles-left-parallax-layers",
        );

        if (parallaxLayersAbout.length) {
    ...
    window.onload = () => {
        ...
        document.querySelectorAll('a[href]:not([href="#"])').forEach((a) => {
            // added :not([href="#"]) to avoid hrefs that ONLY contain the # sign and nothing else
